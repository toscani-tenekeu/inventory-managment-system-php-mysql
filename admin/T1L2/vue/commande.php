    ...
    <div class="overview-boxes">
        <div class="box">
            ...
            <form action="<?= !empty ($_GET['id']) ? "../model/modifCommande.php" : "../model/ajoutCommande.php" ?>" method="post">
                ...
                <label for="id_fournisseur">Supplier</label>
                ...
                <input onchange="setPrix()" value="<?= !empty ($_GET['id']) ? $commande['quantite'] : "" ?>" type="number" name="quantite" id="quantite" placeholder="Please enter the quantity">

                <label for="prix">Price</label>
                <input value="<?= !empty ($_GET['id']) ? $commande['prix'] : "" ?>" type="number" name="prix" id="prix" placeholder="Please enter the price">
                ...
                ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="./factureCommande.php?id_commande=<?= $value['id'] ?>" title="Generate a receipt">
                                        <i class="bx bx-printer" style="font-size: 30px;"></i>
                                    </a>
                                    <a onclick="confirmerSuppressionCom(<?= $value['id'] ?>, <?= $value['idArticle'] ?>, <?= $value['quantite'] ?>)" style="color: #f00;" title="Delete this order"><i class="bx bx-trash" style="font-size: 30px;"></i></a>
                                    <a onclick="confirmerAnnulationCommande(<?= $value['id'] ?>, <?= $value['idArticle'] ?>, <?= $value['quantite'] ?>)" style="color: #0f0;" title="Cancel the order"><i class="bx bx-refresh" style="font-size: 30px;"></i></a>
                                </td>
                            </tr>
                <?php
                        }
